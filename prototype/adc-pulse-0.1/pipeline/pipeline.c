...
bool pipeline_acquisition_callback(struct repeating_timer *t) {
    ...
    absolute_time_t deadline = make_timeout_time_us(PIPELINE_ACQUISITION_US + 10);
    while (dma_channel_is_busy(dma_chan)) {
        if (absolute_time_diff_us(get_absolute_time(), deadline) >= 0) {
            pipeline_state.acquisition_overruns++;
            return true;  // Continue timer but skip this sample
        }
    }
...
bool pipeline_process_acquisition(const uint16_t* raw_data, uint16_t sample_count) {
    ...
    if (processed_count > 0) {
        ...
        pipeline_state.total_samples_processed += sample_count;
    }
...
bool pipeline_start(uint32_t max_iterations) {
    ...
    // Initialize SDIO if not done
    if (!sdio_init()) {
        printf("Pipeline: SDIO initialization failed\n");
        return false;
    }
    
    if (!sdio_card_init()) {
        printf("Pipeline: SD card initialization failed\n");
        return false;
    }
...
bool pipeline_write_buffer_to_sd(pipeline_buffer_t* buffer) {
    ...
    uint32_t blocks_needed = (buffer->size + SDIO_BLOCK_SIZE - 1) / SDIO_BLOCK_SIZE;
    
    // Pad buffer to block boundary if needed
    uint32_t padded_size = blocks_needed * SDIO_BLOCK_SIZE;
    if (buffer->size < padded_size) {
        memset(buffer->data + buffer->size, 0, padded_size - buffer->size);
    }
    
    // Write to SD card
    bool success = sdio_write_multiple_blocks(pipeline_state.sd_block_counter, 
                                             buffer->data, blocks_needed);
    
    if (success) {
        pipeline_state.sd_block_counter += blocks_needed;
    }
