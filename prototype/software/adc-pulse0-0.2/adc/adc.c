...
void pio_adc_init()
{
    ...
    // FORCE ADC to use low-numbered channels (avoid SD card's 8-11)
    // Try channels 0-7 in order until we find one available
    dma_chan = 0;
    while (dma_chan < 8) {
    ...
    if (dma_chan >= 8) {
        printf("ERROR: No DMA channels 0-7 available for ADC!\n");
        return;
    }
