...
static void send_version(void) {
    ...
    int n = snprintf(buf, sizeof(buf),
        ...
        "build:   %s\r\n"
        "release: %s\r\n",
        FW_VERSION, FW_CHANGES, FW_BOARD, FW_CHIP, FW_GIT_HASH, FW_RELEASE_URL);
    if (n > 0) {
        u4rk_usb_write_blocking(buf, (size_t)n, U4RK_CONTROL_TIMEOUT_MS);
    }
}

static void reboot_dfu(void) {
    send_ok("rebooting into USB bootloader (BOOTSEL)");
    sleep_ms(100);          /* let the CDC drain the reply */
    reset_usb_boot(0, 0);   /* does not return */
}

static void send_help(void) {
    send_ok(
        "commands=status|help|version|reboot-dfu|"
        "pulser arm|pulser disarm|"
...
static void process_command(char *line) {
    ...
    if (strcmp(first, "version") == 0 && second == NULL) {
        send_version();
        return;
    }

    if (strcmp(first, "reboot-dfu") == 0 && second == NULL) {
        if (operation_busy()) {
...
#ifdef MUX
    if (strcmp(first, "write") == 0 && second != NULL &&
        strcmp(second, "mux") == 0) {
        char *value_text = strtok_r(NULL, " \t", &save);
        char *extra = strtok_r(NULL, " \t", &save);
        if (operation_busy()) {
            send_error("BUSY", "operation in progress");
        } else if (value_text == NULL || extra != NULL) {
            send_error("ARG", "usage: write mux <hex>");
        } else {
            max14866(value_text);  /* parses the value as hexadecimal */
            send_ok("mux=%s", value_text);
        }
        return;
    }
    if (strcmp(first, "set") == 0 && second != NULL &&
        strcmp(second, "mux") == 0) {
        if (operation_busy()) {
            send_error("BUSY", "operation in progress");
        } else {
    ...
    if (strcmp(first, "clear") == 0 && second != NULL &&
        strcmp(second, "mux") == 0) {
        if (operation_busy()) {
            send_error("BUSY", "operation in progress");
        } else {
